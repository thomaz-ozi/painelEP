<?php
$relatorio_titulo="RELATÓRIO DE ANIMAL - MEDICAMENTOS v:3.0";
switch($_GET['converter']){
	
	case 'xls':
		$filename='lista_animais_medicamentos';
...
?>
<?php
$colname_uni_medicamento = "-1";
if (isset($_GET['cod_animal'])) {
  $colname_uni_medicamento = $_GET['cod_animal'];
}
mysql_select_db($database_connection, $connection);
$query_uni_medicamento = sprintf("SELECT custo_medicamento FROM tbnext_mod_sma_otimizar_relatorio_animais WHERE cod_animal = %s", GetSQLValueString($colname_uni_medicamento, "text"));
$uni_medicamento = mysql_query($query_uni_medicamento, $connection) or die(mysql_error());
$row_uni_medicamento = mysql_fetch_assoc($uni_medicamento);
$totalRows_uni_medicamento = mysql_num_rows($uni_medicamento);
...
mysql_select_db($database_connection, $connection);
$query_relatorio_list_acao = "SELECT   * FROM   vwnext_relatorio_manejo_medicamento  WHERE $sql_cod_animal   ORDER BY $ordem_colunaSQL $ordemSQL";
...
<script type="text/javascript" src="../sistema_funcoes/carregandoDados.js"></script>
...
<table border="0" cellspacing="0" cellpadding="0" class="rel_table">
  <tr class="rel_cabecalho">
    <td colspan="6" class="rel_informacoes" ><?php if($conv==1){ ?>
...
'','toolbar=no,location=no ,menubar=no, status=yes, scrollbars=yes,  resizable=yes, width=1024,height=768')"></div></td>
  ...
  <tr class="rel_titulo">
    <td width="7%" class="coluna_titulo" >MANEJO
    ...
    <td width="5%" class="coluna_titulo" title="PRIMEIRA DATA">DATA
        <?php	$ordem_coluna_nome='2';
	$queryString_1 = sprintf("&cod_animal=".$_GET['cod_animal']."&ordem_coluna=%d%s", $ordem_coluna_nome, $queryString);?>
        <div class="ordem<?php   switch($ordem_coluna){	case $ordem_coluna_nome:	echo $class; break;	default: echo $class_defaul='';	} ?>" onclick="goToURL('<?php printf("%s?ordem=%d%s", $currentPage,$ordem , $queryString_1); ?>')"></div>

    </td>
    <td class="coluna_titulo" title="PRIMEIRO PESO">
      MEDICAMENTO
    ...
    <td width="13%" class="coluna_titulo" title="PRIMEIRO PESO">QUANTIDADE      <?php	$ordem_coluna_nome='4';
	$queryString_1 = sprintf("&cod_animal=".$_GET['cod_animal']."&ordem_coluna=%d%s", $ordem_coluna_nome, $queryString);?></td>
    <td width="21%" class="coluna_titulo" title="PRIMEIRO PESO">CUSTO</td>
    <td width="20%" class="coluna_titulo" title="PRIMEIRO PESO">CUSTO-TOTAL</td>
  </tr> 
  <?php $l=1;?>
  <?php do { ?>
  <tr class="linha<?php   echo $l;  ?>" >
   
      <td class="coluna_conteudo" ><?php echo $id_manejo= $row_relatorio_list_acao['id_manejo']; ?></td>
      <td class="coluna_conteudo"><?php echo $data=$row_relatorio_list_acao['data']; ?></td>
      <td class="coluna_conteudo"><?php echo $row_relatorio_list_acao['nome_medicamentos']; ?></td>
      <td class="coluna_conteudo"><?php echo $row_relatorio_list_acao['qtdd']; ?></td>
      <td class="coluna_conteudo"><?php echo converter_numero_moeda($row_relatorio_list_acao['valor_custo']); ?></td>
      <td class="coluna_conteudo"><?php echo converter_numero_moeda($row_relatorio_list_acao['custo_medicamento']); ?></td>
    </tr>
  <?php   $l++; if($l>2){   $l=1;}?>
...
<table  border="0" align="center" cellpadding="0" cellspacing="0" class="rel_table"  style="text-indent:5px; width:650px;">
  ...
  <tr class="linha1">
    <td>Soma Cuto total</td>
    <td align="right" class="coluna_conteudo"><?php echo converter_numero_moeda($row_uni_medicamento['custo_medicamento']); ?></td>
  </tr>
  <tr class="linha2">
    <td>&nbsp;</td>
...
<?php
mysql_free_result($uni_medicamento);
